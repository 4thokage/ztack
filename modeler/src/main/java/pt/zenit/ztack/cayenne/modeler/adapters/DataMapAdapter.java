...
//            locationProperty = JavaBeanStringPropertyBuilder.create().bean(dataMap).name("map").build();
    ...
    public void sortObjectEntities()
    {
        objectEntityAdapters.sort((entity1, entity2) ->
            {
                return compare(entity1.nameProperty().get(), entity2.nameProperty().get(), false);
            });
    }

    private static <T extends Comparable<? super T>> int compare(final T c1, final T c2, final boolean nullGreater) {
        if (c1 == c2) {
            return 0;
        } else if (c1 == null) {
